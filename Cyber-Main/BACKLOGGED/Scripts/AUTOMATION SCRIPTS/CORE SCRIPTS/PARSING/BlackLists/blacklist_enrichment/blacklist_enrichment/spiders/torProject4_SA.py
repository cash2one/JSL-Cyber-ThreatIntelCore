...
#Note - debugging - some 'list index out of range' errors
...
class ExitNodes(CrawlSpider):
    ...
    #Exit_Address = scrapy.Field()
        ...
        while i < len(fingerPrints):
            ...
            item['Fingerprint_Code'] = fingerPrints[i]
            #item['whoIs'] = urls[i]
...
##        yield item
        ...
        item['Host_Name_Text'] = str(info1[4])
        ...
        if re.match('.*?(?:B|b)itrig.*', platformVersion):  #Bitrig = 1
            item['Platform_Version_Number'] = 1
...
#transfer to csv
#scrapy crawl toripres_1 -o tor_ip_res_1.csv
