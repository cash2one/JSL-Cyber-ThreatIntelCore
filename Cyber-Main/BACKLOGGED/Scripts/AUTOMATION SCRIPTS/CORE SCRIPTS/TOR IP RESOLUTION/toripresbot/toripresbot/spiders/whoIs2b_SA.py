...
#         #upTime
#         upTime = [re.match('.*>(.*?)<\/td>', dataRow[k][4]).group(1) if re.match('.*>(.*?)<\/td>', dataRow[k][4]).group(1) else 'Null' for k, e in enumerate(dataRow)]
#         #hostName
...
# #1 scrapy crawl whois2 -o tor_exit_addresses6a.csv
