...
def updatesearch(date):
    ...
    if count > 0:
        if pgp == False:
            os.system(
                'mail  -s "WhoDat Registration Alert ( ' + date + ' )" ' + notifyemail + ' -- -f ' + sendingemail + ' < ' + updatepath + 'newdomains.txt')
        else:
            os.system(
                'gpg --trust-model always -ea -r ' + notifyemail + ' -o - ' + updatepath + 'newdomains.txt  | mail  -s "WhoDat Registration Alert ( ' + date + ' )" ' + notifyemail + ' -- -f ' + sendingemail)
...
def downloads(date):
    ...
    if not os.path.isfile(updatepath + date + "com" + ".csv.gz"):
        os.system(
            'echo "Error downloading updates for WhoDat!" |  mail  -s "[!] WhoDat Update Error " ' + notifyemail + '  -- -f ' + sendingemail)
...
def insertfile(date):
    for tld in tlds:
        os.system(
            'mongoimport --collection whois --file ' + updatepath + date + tld + '.done.csv  --type csv --headerline --upsert --upsertFields domainName')
