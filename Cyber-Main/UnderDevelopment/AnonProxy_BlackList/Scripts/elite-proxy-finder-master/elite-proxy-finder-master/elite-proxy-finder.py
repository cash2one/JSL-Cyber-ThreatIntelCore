...
def parse_args():
    ...
    parser.add_argument('-s', '--show',
                        help='Show this number of results. Example: "-s 5" will show the 5 fastest proxies then stop')
    parser.add_argument('-a', '--all', help='Show all proxy results including the ones that failed 1 of the 3 tests',
                        action='store_true')
    parser.add_argument('-q', '--quiet', help='Only print the IP:port of the fastest proxies that pass all the tests',
                        action='store_true')
...
class find_http_proxy():
    ...
    def proxy_checker_req(self, proxy):
        ...
        urls = ['http://danmcinerney.org/ip-test.php', 'http://myip.dnsdynamic.org',
