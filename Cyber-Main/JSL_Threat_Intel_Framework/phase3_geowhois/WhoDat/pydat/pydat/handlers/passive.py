...
def __request_pdns_dnsdb(domain, absolute, rrtypes, limit, pretty):
    ...
    for rrtype in rrtypes:
        url = "https://api.dnsdb.info/lookup/rrset/name/" + wildcard + urllib.quote(
            domain) + "/" + rrtype + "/?limit=" + str(limit)
        try:
            r = requests.get(url,
...
def __request_pdns_reverse_dnsdb(key, value, rrtypes, limit, pretty):
    ...
    results['data'] = {}
    for rrtype in rrtypes:
        url = "https://api.dnsdb.info/lookup/rdata/" + key + "/" + urllib.quote(value) + "/" + rrtype + "?limit=" + str(
            limit)
        try:
            r = requests.get(url,
                             proxies=settings.PROXIES,
